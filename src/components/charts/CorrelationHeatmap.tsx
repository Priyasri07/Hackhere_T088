...
export const CorrelationHeatmap: React.FC = () => {
  ...
  if (selectedAssets.length < 2 || !correlationData) {
    return (
      <div className="flex flex-col items-center justify-center p-8 bg-slate-900/40 rounded-xl border border-slate-800 text-center min-h-[220px]">
        <div className="w-10 h-10 rounded-full bg-sky-500/10 border border-sky-500/20 flex items-center justify-center text-sky-400 mb-3">
          <Layers className="w-5 h-5" />
        </div>
        <h4 className="text-sm font-semibold text-slate-200 mb-1">Correlation Analysis</h4>
        <p className="text-xs text-slate-400 max-w-sm">
          Select at least <strong className="text-sky-300">2 assets</strong> from the top global bar to calculate return-based Pearson correlation.
        </p>
      </div>
    );
  }
  ...
  const getCellColor = (val: number, isDiagonal: boolean) => {
    if (isDiagonal) return 'bg-sky-500/20 text-sky-300 border-sky-500/40 font-bold';
    if (val >= 0.7) return 'bg-emerald-500/30 text-emerald-200 border-emerald-500/50 font-bold';
    if (val >= 0.3) return 'bg-emerald-500/15 text-emerald-300 border-emerald-500/30';
    if (val >= 0.0) return 'bg-slate-800/80 text-slate-200 border-slate-700';
    ...
    <div className="w-full">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2 text-[10px] text-slate-400 font-mono">
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-emerald-500/50"></span> High +</span>
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-slate-700"></span> Uncorrelated</span>
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-rose-500/50"></span> Inverse -</span>
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr>
              <th className="p-2.5 text-left text-slate-500 font-mono text-[11px]">Asset</th>
              {assetIds.map(id => {
          ...
          <tbody>
            {assetIds.map((rowId, i) => {
              ...
              return (
                <tr key={rowId} className="border-t border-slate-800/60">
