...
export const StrategyLabPage: React.FC = () => {
  ...
  const STRATEGIES: { id: StrategyType; name: string; desc: string; icon: string }[] = [
    {
      id: 'SMA_CROSSOVER',
      name: 'SMA Crossover',
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="quant-card p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-sky-500 to-cyan-500 hover:from-sky-400 hover:to-cyan-400 text-slate-950 text-sm font-bold rounded-xl shadow-lg shadow-sky-500/25 transition-all cursor-pointer active:scale-95"
        >
      ...
      {validationError && (
        <div className="flex items-center justify-between bg-rose-500/10 border border-rose-500/30 p-3.5 rounded-xl text-xs text-rose-300">
          ...
          <button
            ...
            className="px-3 py-1 bg-rose-500/20 hover:bg-rose-500/30 text-rose-200 font-semibold rounded border border-rose-500/30"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="quant-card p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800">
                  <div className="flex justify-between text-xs mb-2">
                    <span className="text-slate-300 font-medium">Fast SMA Window</span>
                    <span className="font-mono text-sky-400 font-bold">{strategyParams.shortWindow || 20} days</span>
                  </div>
                  <input
                    type="range"
                    min="5"
                ...
                <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800">
                  <div className="flex justify-between text-xs mb-2">
                    <span className="text-slate-300 font-medium">Slow SMA Window</span>
                    <span className="font-mono text-sky-400 font-bold">{strategyParams.longWindow || 50} days</span>
                  </div>
                  <input
                    type="range"
                    min="20"
            ...
            {strategyParams.type === 'EMA_TREND' && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800">
                  <div className="flex justify-between text-xs mb-2">
                    <span className="text-slate-300 font-medium">Fast EMA (e.g. 12)</span>
                    <span className="font-mono text-purple-400 font-bold">{strategyParams.shortWindow || 12} days</span>
                  </div>
                  <input
                    type="range"
                    min="5"
                ...
                <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800">
                  <div className="flex justify-between text-xs mb-2">
                    <span className="text-slate-300 font-medium">Slow EMA (e.g. 26)</span>
                    <span className="font-mono text-purple-400 font-bold">{strategyParams.longWindow || 26} days</span>
                  </div>
                  <input
                    type="range"
                    min="20"
            ...
            {strategyParams.type === 'MOMENTUM' && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800">
                  <div className="flex justify-between text-xs mb-2">
                    <span className="text-slate-300 font-medium">Lookback Period</span>
                    <span className="font-mono text-emerald-400 font-bold">{strategyParams.momentumLookback || 20} days</span>
                  </div>
                  <input
                    type="range"
                    min="5"
                ...
                <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800">
                  <div className="flex justify-between text-xs mb-2">
                    <span className="text-slate-300 font-medium">Hurdle Threshold</span>
                    <span className="font-mono text-emerald-400 font-bold">+{strategyParams.momentumThreshold || 1.5}%</span>
                  </div>
                  <input
                    type="range"
                    min="0.1"
            ...
            {strategyParams.type === 'MEAN_REVERSION' && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800">
                  <div className="flex justify-between text-xs mb-2">
                    <span className="text-slate-300 font-medium">Z-Score Window</span>
                    <span className="font-mono text-amber-400 font-bold">{strategyParams.mrWindow || 20} days</span>
                  </div>
                  <input
                    type="range"
                    min="10"
                ...
                <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800">
                  <div className="flex justify-between text-xs mb-2">
                    <span className="text-slate-300 font-medium">Band Std Deviation (σ)</span>
                    <span className="font-mono text-amber-400 font-bold">{strategyParams.mrStdDev || 2.0} σ</span>
                  </div>
                  <input
                    type="range"
                    min="1.0"
          ...
          <div className="quant-card p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-slate-900/80 p-3.5 rounded-xl border border-slate-800">
                ...
                <div className="flex items-center gap-1.5 font-mono text-sm text-slate-200">
              ...
              <div className="bg-slate-900/80 p-3.5 rounded-xl border border-slate-800">
                <div className="text-xs text-slate-400 mb-1">Position Size (% Capital)</div>
                <div className="flex items-center gap-1.5 font-mono text-sm text-slate-200">
              ...
              <div className="bg-slate-900/80 p-3.5 rounded-xl border border-slate-800">
                <div className="text-xs text-slate-400 mb-1">Transaction Fee (bps)</div>
                <div className="flex items-center gap-1.5 font-mono text-sm text-sky-400">
                  ...
                  <input
                    ...
                    className="bg-transparent font-bold focus:outline-none w-full"
                  />
                  <span className="text-[10px] text-slate-500 font-sans">({((strategyParams.transactionCostBps || 10) / 100).toFixed(2)}%)</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="quant-card p-5">
            ...
            {isSingleAsset ? (
              <div className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 text-xs">
                <div className="text-slate-400">Single Target Asset:</div>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  {backtestMode === 'PORTFOLIO' && <CheckCircle2 className="w-4 h-4 text-sky-400" />}
                </button>
                ...
                >
                  ...
                  {backtestMode === 'INDIVIDUAL' && <CheckCircle2 className="w-4 h-4 text-sky-400" />}
                </button>

                {backtestMode === 'INDIVIDUAL' && (
                  <div className="pt-2">
                    <label className="text-[11px] text-slate-400 uppercase font-semibold">Choose Asset:</label>
                    <select
                      value={targetSingleAsset}
                      onChange={e => setTargetSingleAsset(e.target.value as AssetId)}
                      className="mt-1 w-full bg-slate-900 border border-slate-800 p-2.5 rounded-lg text-xs font-semibold text-slate-200 focus:outline-none"
                    >
          ...
          {!isSingleAsset && backtestMode === 'PORTFOLIO' && (
            <div className="quant-card p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  onClick={normalizeWeights}
                  className="text-[11px] px-2 py-1 rounded bg-slate-800 hover:bg-slate-700 text-sky-300 border border-slate-700 transition-colors cursor-pointer"
              ...
              <div className="space-y-4">
                {selectedAssets.map(assetId => {
                  ...
                  return (
                    <div key={assetId} className="bg-slate-900/60 p-3 rounded-xl border border-slate-800/80">
          ...
          {/* Quick Launch Card */}
          <div className="quant-card p-5 bg-gradient-to-b from-[#0d1527] to-[#0b101e] border-sky-500/20">
            ...
            <button
              onClick={handleRunBacktest}
              className="w-full py-3 bg-sky-600 hover:bg-sky-500 text-white font-bold text-xs rounded-xl shadow-md transition-all cursor-pointer flex items-center justify-center gap-2"
            >
              <Play className="w-4 h-4 fill-current" />
